...
def train_model():    
    ...
    lstm_layer = tf.keras.layers.RNN(tf.keras.layers.LSTMCell(64), input_shape=(None, 600))
    model = tf.keras.models.Sequential(
        [lstm_layer,
         tf.keras.layers.BatchNormalization(),
         tf.keras.layers.Dense(len(m_signals), activation='softmax')])
    model.summary()
    print("Обучение сети...")
    ...
    f = open("labels_rnn.bin", "wb")
    f.write(pickle.dumps(m_signals))
    f.close()
    print("Успешно")    
    f = open("train_history_rnn.bin", "wb")
...
def predictions(signal):
    ...
    try:
        ...
        model.summary()        
        lb = pickle.loads(open("labels_rnn.bin", "rb").read())
        print("Успешно")
          
        #Определание типа сигнала

        pred = model.predict(np.expand_dims(signal,axis=1))
